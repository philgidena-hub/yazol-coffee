      ...
      >
        ...
        >
            <div className="overflow-y-auto max-h-[90vh] sm:max-h-[85vh]">
              ...
              <div className="sticky bottom-0 bg-white border-t border-black/5 px-5 sm:px-6 py-4 pb-safe-extra">
                <div className="flex items-center gap-4">
                  {/* Quantity selector */}
                  <div className="flex items-center bg-surface rounded-xl border border-black/5">
                    <button
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="px-3.5 py-3 text-brown/50 hover:text-brown transition-colors touch-target flex items-center justify-center"
                    >
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M20 12H4" />
                      </svg>
                    </button>
                    <span className="px-3 text-brown font-body font-medium text-sm tabular-nums">{quantity}</span>
                    <button
